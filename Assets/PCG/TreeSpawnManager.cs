...
public class TreeSpawnManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for(int i = 0; i < HowManyTrees; i++){
            ...
            try{
                GameObject tree = Instantiate(Tree[RandomSpawn],  GeneratedPosition(), Quaternion.identity);// here are spawned but the y needs to be modified correctly, so that trees will appear on the ground and not floating
